            ...
            switch ((unsigned char)entry->attributes[0]) {
                ...
                case 0x20:
                    if (entry->filename[0] != 0x0F) {
                        ...
                        fseek(in, (entry->cluster_lowbytes_address - 2) * 512, SEEK_SET); // Calcular la posición del primer cluster
                        unsigned int fileSize = entry->size_of_file;
                        char *fileContent = (char *)malloc(fileSize);
                        fread(fileContent, fileSize, 1, in);
                        // Imprimir el contenido del archivo
                        printf("Contenido del archivo:\n%s\n", fileContent);
                        free(fileContent); // Liberar memoria del contenido del archivo
                    }
                    break;
            }
    }
